 ...
 * сообщением нельзя.
 */
function sessionLife({ entry, session, assignment, config, now }) {
  if (!session) {
    // Этап без сессии — это ДВА разных случая, и мерить их одной меркой
    // нельзя. Различает их выданный слот.
    //
    // Слота нет — сессии не было и никто её не готовит: этап только начался,
    // ждать некого, работу надо давать сейчас. Прежде ждали и здесь, отмеряя
    // выдержку от смены состояния порогом смерти, — и каждый этап после
    // первого начинался на полчаса позже, чем мог бы. При восьми этапах это
    // четыре часа простоя на задачу, не потраченных ни на какую работу.
    //
    // Проверено 28.08.2026: задача 0025 переехала в разбор около 12:06,
    // а действие получила в 12:41 — ровно по истечении получаса.
    if (!assignment) return 'сессии нет';

    // Слот выдан — исполнитель просто ещё не проснулся. Он и оркестратор
    // ходят по расписанию независимо, и порядок между ними не задан: между
    // выдачей слота и первым пробуждением проходит до целого интервала.
    // Продолжатель, порождённый в эту щель, ничего не чинит, зато тратит
    // попытку, — а их всего две, и после второй задача ждёт человека.
    //
    // Отсчёт идёт от слота, а не от смены состояния: слот и есть след
    // того, что работу уже кому-то поручили. `startedAt` пишет проснувшийся
    // исполнитель, поэтому он точнее выдачи, когда есть.
    const since = assignment.startedAt ?? assignment.assignedAt;
    const waitingFor = minutesBetween(now, since);
    if (waitingFor !== null && waitingFor < config.deadAfterMinutes) return 'жива';
    return 'сессии нет';
  }
  // Сессия, ЗАКОНЧИВШАЯСЯ на глазах у нескольких снимков подряд.
  //
  // Признак «не идёт» сам по себе ненадёжен: снимок ловит сессию между
  // ходами, и однажды оркестратор похоронил живую за двадцать пять секунд
  // ДО её же последней активности — прогон досчитался, а задача сгорела.
  // Отсюда общее правило ниже: верить не признаку, а сроку молчания.
  //
  // Но признак становится надёжным, когда держится ЧЕРЕЗ снимки. Сессия
  // между ходами хотя бы на одном окажется идущей либо сдвинет отметку
  // активности; закончившаяся не сделает ни того, ни другого никогда.
  // Снимок копит это в `notRunningSince`, здесь остаётся только сверить
  // срок — вдвое короче смертного, но и опирается он на два наблюдения,
  // а не на одно.
  //
  // Цена прежнего единого срока была не теоретической: 28.08.2026 сессия
  // по 0005 прожила тридцать пять секунд, а слот единственного исполнителя
  // держала тридцать одну минуту.
  const endedFor = minutesBetween(now, session.notRunningSince);
  if (!session.isRunning && endedFor !== null && endedFor >= config.endedAfterMinutes) {
    return 'сессия завершилась без отчёта';
  }

  // Срок молчания один на все прочие случаи.
 ...
 * @param {object}   state.occupancy занятость слотов: `{ имяСлота: назначение|null }`
...
export function scan(state) {
  const {
    ...
    reports = [],
    sessions = [],
    // Занятость слотов сканеру нужна, чтобы отличить «сессии ещё не было»
    // от «сессию ждут». Раскладываются слоты позже него, но ЧИТАЮТСЯ они
    // до: оркестратор кладёт занятость в тот же расклад, что и всё прочее.
    occupancy = {},
  ...
  for (const task of sessionsKnown ? tasks : []) {
    ...
    // Назначение берётся строго на НЫНЕШНИЙ этап: слот, оставшийся от
    // прошлого, — это протухшее назначение, а не признак начатой работы.
    // Его освобождает сверка, а здесь он значит ровно то же, что и его
    // отсутствие: работу никому не поручили.
    const assignment = Object.values(occupancy).find(
      (item) => item?.taskId === task.id && item.stage === task.status,
    );

    const life = sessionLife({ entry, session, assignment, config, now });
