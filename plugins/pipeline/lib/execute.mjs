...
async function startStage(action, io) {
  ...
  if (!push.ok) {
    ...
    if (push.outcome === 'conflict') {
      ...
      // хвост главной ветки запрёт записи всему конвейеру, — а доске
      // прибирать нечего, там гонку решает тег захвата.
      io.undoSave?.(push, releaseClaim(claimed.task));
      return { result: 'raced', why: 'задачу занял кто-то другой' };
    }

    // Остальное — это либо неудача до записи, которую хранилище уже
    // прибрало, либо годный коммит без отправки. Годный коммит и есть
