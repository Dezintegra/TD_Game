 ...
 * а откладывается: агент кладёт запись в `balance/queue.json`, а разбирает
 * очередь ночная джоба на чужом железе. Правила — в CLAUDE.md, правило 8;
 * формат записи — в `balance/README.md`.
 *
...
const QUEUE_PATH = join(repoRoot, 'balance', 'queue.json');
...
const readQueue = () => {
  if (!existsSync(QUEUE_PATH)) die(`очереди нет: ${QUEUE_PATH}`);

  let parsed;
  try {
    parsed = JSON.parse(readFileSync(QUEUE_PATH, 'utf8'));
  } catch (error) {
    die(`очередь не разбирается как JSON: ${error.message}`);
  }

  if (!Array.isArray(parsed)) die('очередь должна быть массивом записей');
...
const proposals = (shifted) =>
  [
    ...
    '- вопрос **закрыт** — уберите запись из `balance/queue.json`, иначе',
