...
// Порог для занятости ЗА ПРОГОН. Число другое, потому что и величина
// другая: сюда входит сам замер — Playwright, Chromium, свой клиент
// и свой сервер, — а `BUSY_LIMIT` мерился, когда ничего этого
// не запущено. Одним числом их мерить нельзя, и замысел запрещал
// не это, а два числа с ОДНИМ смыслом.
//
// Значение подобрано замером 25.08.2026: два прогона подряд на тихой
// машине (занятость до прогона 6 % и 7 %) дали за прогон медиану 32 %
// и 31 %. То есть сам замер съедает почти весь прежний порог, и оставь
// его здесь — обычный прогон объявлялся бы негодным от малейшего
// шороха рядом. Вдвое выше собственной цены и ниже самого тихого
// из прогонов под искусственной нагрузкой (0,65).
export const RUN_BUSY_LIMIT = 0.6;
...
/**
 * Занятость записи словами — с оговоркой, какая именно.
 *
 * Две величины сливать в одну строку нельзя: у записи нового образца
 * есть занятость за прогон (в неё входит сам замер), у старой — только
 * та, что снята до начала. Числа разного смысла, и подписаны они
 * поэтому по-разному.
 */
const busyWords = (entry) =>
  entry?.busyMedian === undefined
    ? `занятость до прогона ${percent(entry?.busy)}`
    : `занятость за прогон ${percent(entry.busyMedian)}`;
...
export const comparability = (entry) => {
  ...
  // Две занятости, два вопроса, два порога.
  //
  // «До прогона» отвечает на «стоит ли начинать» и меряется, когда
  // ничего своего ещё не запущено. «За прогон» отвечает на «под чем
  // снято» и включает сам замер. Числа у них разные не по недосмотру,
  // а потому что разные величины.
  const before = entry?.busy;
  if (typeof before === 'number' && before > BUSY_LIMIT) {
    reasons.push(`машина была занята ещё до прогона: ${percent(before)}`);
  }

  const load = entry?.busyMedian;
  if (typeof load === 'number' && load > RUN_BUSY_LIMIT) {
    reasons.push(
      `занятость за прогон ${percent(load)} выше порога ${percent(RUN_BUSY_LIMIT)}` +
        ' — рядом шло что-то ещё',
    );
  }
...
export const lastComparable = (kind, entries = readLog()) => {
  ...
  for (let index = same.length - 1; index >= 0; index -= 1) {
    ...
    // Записи старого образца годность не судит вовсе — данных для этого
    // в них нет. Но занятость до прогона в них записана, и говорит она
    // ровно то, что нужно: шло рядом что-нибудь или нет. Именно ею
    // отсеиваются четыре прогона 25.08.2026 под искусственной нагрузкой.
    const before = candidate.busy;
    if (typeof before === 'number' && before > BUSY_LIMIT) continue;
...
export const recordEntry = ({
  ...
  if (previous === undefined) {
    ...
    const basis = `«было» — ${previous.commit} от ${when}, ${busyWords(previous)}`;
