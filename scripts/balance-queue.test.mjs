...
describe('настоящий бэклог этого репозитория', () => {
  it('разбирается, и каждый заказанный прогон истолковывается', () => {
    // Проверка на живых данных: если запись прогона испорчена, здесь это
    // видно сразу.
    //
    // Непустоты очередь НЕ обязана. Прежде тест её требовал — и однажды
    // покраснел на ровном месте: конвейер разобрал все тринадцать арен,
    // очередь честно опустела, а `main` встал на три прогона подряд.
    // Пустая очередь означает «всё посчитано», а не «что-то сломалось»;
    // после переезда бэклога на доску она пуста всегда, потому что читать
    // здесь уже нечего.
