...
describe('настоящий бэклог этого репозитория', () => {
  it('разбирается, и каждая собранная запись годна', () => {
    // Проверка на живых данных: если бэклог что-то испортил, здесь это
    // видно сразу. Но проверяется ГОДНОСТЬ собранного, а не его количество.
    //
    // Требование «хотя бы одна запись» тут стояло и было ошибкой: длина
    // очереди — это не свойство системы, а свойство минуты. Конвейер
    // разбирает прогоны сам, и однажды ночью он довёл число ждущих
    // до нуля — после чего красная проверка встала поперёк всех чужих
    // pull request, ничего при этом не найдя. Пустая очередь означает
    // «всё померено», а это хорошая новость, а не поломка.
    //
    // После переезда бэклога на доску очередь пуста всегда: читать здесь
    // уже нечего, и заказанные прогоны конвейер берёт с карточек.
