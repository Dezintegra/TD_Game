# pipeline-self-update-scope Specification

## Purpose

Сохранять доступные слоты конвейера при изменениях файлов, которые уже работающий супервизор не загружает, не ослабляя безопасный перезапуск после изменения его исполняемого кода.
