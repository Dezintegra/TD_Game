...
Следующий допуск на разработку/сборку и опыт должен ссылаться на отправленный проект, точный патч/build manifest, source-coverage review, результаты отрицательных проверок и текущие полномочия Windows-хозяина. Аудит проекта не даёт автоматического разрешения на эти действия. Реализацию поставщика следует явно поручить до продолжения зависимых live acceptance; не создавать в этом design обратную зависимость на 0370 и не запускать её collector самостоятельно.
