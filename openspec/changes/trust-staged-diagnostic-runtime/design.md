...
1. The diagnostic path accepts an explicit entrypoint attestation; ordinary `supervisorIdentity` callers retain their main-root default. The runtime derives its entrypoint from its own module location, not the descriptor or requester. The client validates a descriptor entrypoint only after confirming that it is the canonical `supervisor/bin/supervise.mjs` inside the main checkout or a registered clean worktree of the same Git common directory. Exact command-line matching and live root-lock PID remain mandatory. This avoids a PID-only bypass and keeps other watchdog/stop checks unchanged.
