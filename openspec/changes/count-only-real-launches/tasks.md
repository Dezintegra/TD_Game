...
- [x] `supervisor/lib/scan.mjs`, раздел 6: считать свободные места как
...
- [x] Проверку исчерпанных пределов оставить **выше** проверки мест:
      задача, которую дальше вести нельзя, останавливается и на занятой
      машине.
- [x] Тесты в `supervisor/lib/scan.test.mjs`: при занятом единственном
...
- [x] `manage/README.md`: к двум путям в «Разбор ошибки» дописать третий —
      этап не порождается; сказать, что теснота попыток не тратит.
- [x] `supervisor/README.md` (около строки 151): рядом с пределом
      продолжений назвать предел несостоявшихся запусков и разницу между
      ними.
- [x] `supervisor/skills/postmortem.md`: в перечень причин остановки
