...
- [x] 2.1 Разбирать в заявке поле `area: "pipeline"` строго
      ...
      несёт `blocking`
- [x] 2.2 Дать правило `area` скиллам, подающим заявки: `design`, `audit`,
...
- [x] 3.2 Вердикт разбора: `supervisor/lib/recovery.mjs` собирает
      ...
      человек»; предел `maxAutoReturns` в `config/defaults.mjs`. Проверяемо: `recovery.test.mjs` — строгость `causedBy`,
      отбрасывание чужого идентификатора, предел; `execute.test.mjs` —
      задача в ошибке несёт `fixedBy` с номером заведённой задачи
- [x] 3.3 Действие `return-task`: сканер назначает его при закрытых
      ...
      действие в перечне пишущих в главную ветку (`cycle.mjs`). Проверяемо: `scan.test.mjs` —
      четыре сценария из спецификации; `execute.test.mjs` — возврат
      меняет состояние, счёт и журнал, `forgetSession` позван
- [x] 3.4 Скилл разбора: поля `causedBy` и `fixedBy`, правило «конвейер
...
- [x] 4.2 Переданный замок: `lockVerdict` берёт замок с собственным
      номером (`supervisor/lib/lock.mjs`). Проверяемо: `lock.test.mjs`
- [x] 4.3 Провести в супервизор (`bin/supervise.mjs`): `git fetch`
