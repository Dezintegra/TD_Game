...
Цель — проверяемая поставка источника B для 0.153.4, готовая к отдельному решению об активации; после допуска — пригодные первичные факты либо точная подтверждённая преграда. Исполняемый первый объём уже разрешён владельцем: получить upstream, создать патч, собрать и испытать на подменённых ОС/IPC-границах. Это последующая реализация после аудита данного изменения, а не действия текущего design.
...
В `supervisor/instrumentation/codex-refresh/` хранятся `source-manifest.json`, `refresh-boundary.patch`, `build-recipe.json`, `prepare.mjs`, `build.mjs`, `README.md`, `coverage.md` и очищенные receipts в `receipts/`. JSON содержит реальные измеренные значения после implement, не правдоподобные placeholders. Все generated source/build/test материалы находятся в собственной `.matchlog/0372-refresh/`: upstream checkout, два независимых build roots, project-local caches, negative-control outputs, packaged executables. Никакой записи в `.pipeline/codex-runtime`, дерево 0371 или дерево 0370 по первому допуску.
...
Для Node-части: `pnpm install --frozen-lockfile --prefer-offline`, затем `pnpm test:pipeline lib/refresh-boundary-source.test.mjs instrumentation/codex-refresh/prepare.test.mjs instrumentation/codex-refresh/build.test.mjs`. Создаваемая тонкая `.matchlog/0372-refresh/run.mjs` вызывает версионированный driver только режимами prepare/check/negative/build/reproduce/package; не содержит произвольного shell eval или host activation. Это штатная покрытая форма `node .matchlog/0372-refresh/run.mjs <режим>` из stage-settings, не обход отказа Cargo. При реальном запрете вложенного действия исполнение останавливается с точной ошибкой; не расширять allowlist или profile ради успеха. Каждая операция выдаёт receipt; отсутствие receipt/тестов — не успех. Полный verify, игровые сборки/матчи, арена не планируются.

### 6. Последовательная передача и достижимое завершение

**Уже разрешено:** разработать патч, получить upstream, собрать кандидата и проверить fake boundaries. Основание — назначение владельца 2026-09-24 и принятый аудит 0371. Ни текущий design, ни первый implement не исполняют живое окно. Статус design этой карточки может быть done после отправки артефактов; это только переход к независимому audit.

**Результат первого implement:** точные patch/source/recipe/schema/coverage hashes; два build receipts с binary hashes и absolute paths; отрицательные результаты и mutation controls; список неснятых технических фактов; `activation-request.md` с candidate buildId, условиями и запретами. В отчёте предъявляется этот пакет для второго допуска. Все checkboxes первого объёма могут быть закрыты, но это не разрешает объявить всю 0372/0370 завершённой: при готовом пакете следующий исход — question о конкретной активации, без повторного вопроса о разработке или A/B. Варианты для владельца: разрешить конкретное ограниченное окно после подтверждения условий либо оставить сборку неактивной; рекомендация разрешить только при полном пакете и технически готовом collector. Не включать ожидание ответа/CI или чужой live launch в исполняемые tasks.

Если второго допуска недостаёт по технической причине, пакет называет конкретный факт, способ получения и владельца:

| Недостающий факт | Способ получения | Ответственный |
| --- | --- | --- |
| Патч/сборка/coverage/negative receipts | Первый допуск, производственные тесты, два чистых build roots | Исполнитель 0372; нельзя переложить ещё не выполненную разработку на владельца |
| Точный source diff итоговой 0371 | Read-only сравнение immutable SHA с merged PR; повторить перед активацией | Исполнитель 0372 |
| Observer adapter/collector admission/locks готовы | Передать библиотеку/fixtures, получить проверку существующей композиции 0370; не запускать её из 0372 | Исполнитель/хозяин 0370; без обратной зависимости или дублирующей карточки |
| Полномочия/контекст Windows-хозяина и сохранение артефактов | Статическая read-only сверка заданного host plan, путей и доступности, без setup/token query | Штатный Windows-хозяин по маршруту 0370 |
| Фактически выбранный helper и transient token | Только отдельно разрешённое окно, process-handle handshake и inline observations | Штатный хозяин и collector 0370; не условие начала разработки |

**После второго допуска:** уполномоченный хозяин активирует ровно одобренный buildId без перезаписи stock binaries; выбирает его в плане существующего collector. Сначала сохранены stock paths/hashes и возвратный план, подтверждены locks/budget/ownership и все условия 0370. Нельзя менять `.pipeline` работающего супервизора из дочернего этапа. Окно запускается владельцем маршрута 0370, не вторым launcher из этой библиотеки. Отказ helper provenance/handshake до первой команды даёт not-run; потеря после старта даёт partial и штатный bounded drain, не успешный release без доказанного выхода. Обновление контракта 0370 выполняет её владелец в существующей карточке, не через новое ребро 0372 → 0370.

Источник остаётся тем же предметом 0372. После поступления конкретного допуска/материалов план этого изменения дополняется только разрешённой приёмкой: проверить пакет, hashes, UTC, первичные edges, direct ACL result/token и итоги read/Git; сохранить `live-validation.md` либо `technical-barrier.md`. Это не новая карточка и не новая дельта. Проверка полученных материалов выполнима read-only, не требует самовольного запуска.

Handoff содержит absolute paths, SHA-256, UTC и ссылки на конкретные primary records цепи invocation → call/attempt → refresh leader/joiner → helper/thread → aclOp/token/result → read/Git outcome. Техническая преграда содержит точную attempted boundary/API/error, reference, влияние на три вердикта и следующий доступный способ получения факта. «Команда прошла», valid JSON и synthetic receipt не заменяют причинную приёмку. Source readiness, collector completeness и repair 0366 — разные статусы. Неполнота не означает ремонт и не снимает incident gate.

Предусмотренный постановкой альтернативный результат «точная подтверждённая техническая преграда» не требует ждать завершения зависимой 0370: если пакет источника готов, но её существующий collector ещё не допускает окно, handoff фиксирует именно проверенный статус/отсутствующий adapter и передаёт пакет её владельцу. Нельзя превращать это в обратное ребро, бесконечное ожидание или объявление живого опыта состоявшимся. Сам факт отсутствия второго разрешения не техническая преграда: для него возвращается конкретный вопрос с готовым пакетом. Решение о приёмке переданных материалов и продолжении 0370 остаётся её анализу; источник не снимает зависимости и не правит доску.
...
Продуктового вопроса для design и первого допуска нет. Неизвестны фактическая собираемость/воспроизводимость в разрешённой рабочей области, полное покрытие upstream graph, native handle transport и token stability на хозяине; первые три устанавливает реализация, последний — позднее окно. Сейчас не заявляются sourceAvailable, live completeness или ремонт. Второй допуск запрашивается только с конкретным проверенным пакетом, не заранее.
