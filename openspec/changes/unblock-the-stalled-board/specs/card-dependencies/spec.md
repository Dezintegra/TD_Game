...
При сохранении карточки в состоянии `closed` супервизор SHALL разрешить каждое
ребро ожидания, названное в `dependsOn` или `recovery.fixedBy` других карточек.
Закрытие дроблением с непустым `splitInto` SHALL переводить ребро на все части;
всякое иное закрытие SHALL снимать ребро. Разрешение SHALL выполняться
до того, как мерка запуска в следующий раз прочитает ждущую карточку.
...
#### Scenario: Закрытие дроблением переводит ожидание на части

- **WHEN** предшественник закрыт с непустым `splitInto`
- **THEN** ждущие карточки ссылаются на все части вместо закрытой, а обоснование называет дробление и перечень частей
