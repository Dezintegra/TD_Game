...
export function stageCommand({ assignment, prompt, config, root, home = root }) {
  ...
  // значило бы гадать.
  //
  // Поток событий (`stream-json`) отличается от однократного ответа
  // не составом, а временем: те же поля приходят последним событием,
  // но до него видно, что этап делает прямо сейчас. Ключ `--verbose`
  // приложение при этом требует — без него поток не включается.
  const format = config.stageOutputFormat ?? 'stream-json';
