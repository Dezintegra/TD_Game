...
export function stagePrompt({
  ...
  if (
    ...
    lines.push(
      ...
      'passed — boolean; evidence — непустая строка либо непустой массив непустых строк (пробелы не считаются свидетельством).',
      'Примеры evidence: "Команда выполнена" или ["Команда выполнена", "Результат проверен"]. Все элементы сохраняются через LF без обрезки и удаления повторов.',
