...
describe('решение о запуске', () => {
  ...
  it('и названо, что там набрать', () => {
    const decision = decideLaunch({ root: HERE, gitFile: worktreeLink() });
    // Дословно тот вид, который закреплён сторожом в наборе тестов проекта:
    // сценарии корневого описания пакета доказанно разрешаются по `pnpm run`.
    // Короткая запись без `run` работает у пакетного менеджера тоже, но ничем
    // не проверена, и обещать в отказе непроверенное нельзя — читающий его
    // и так уже промахнулся каталогом.
    expect(decision.message).toContain('pnpm run supervisor');
  });
