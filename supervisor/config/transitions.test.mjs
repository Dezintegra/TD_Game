...
describe('этапы и скиллы', () => {
  ...
  it('ответ о поведении openspec archive назван с версией и датой пробы', () => {
    ...
    const sources = [['скилл проработки', skillText('design')]];
    // Памятка проекта лежит вне каталога инструмента: супервизор переносим,
    // и в чужом дереве её может не быть вовсе. Здесь она есть всегда,
    // и молчаливого пропуска этот guard не даёт — проверка просто
    // не применима к переносимой копии.
    if (existsSync(claudeMd)) sources.push(['памятка проекта', readFileSync(claudeMd, 'utf8')]);
    expect(sources).toHaveLength(2);

    for (const [name, text] of sources) {
      const window = near(text);
      expect(window, `${name}: ключ --skip-specs не упомянут вовсе`).not.toBeNull();
      expect(window, `${name}: рядом с --skip-specs нет версии OpenSpec`).toMatch(/\d+\.\d+\.\d+/);
