 ...
 * осознанно короткая выборка из скилла (`gh pr checks` шага 3 `review.md`
 * в неё не входит), а весь путь ревью покрыт одним приставочным правилом
 * `gh pr:*`, при котором выборка и полный перечень неразличимы. Номер здесь
 * был сдвинут на единицу при живом тексте и выправлен вместе со сторожем
 * ссылок: шагом 2 `review.md` зовётся `gh pr view`, а не `gh pr checks`.
 * Перекраивать выборку здесь нельзя
 ...
 * `deploy.md`», «Строки шагов 5 и 9 `deploy.md`»). Каждое число идёт
 * отдельной ссылкой:
...
const stepLinkFaults = (files, stepsByFile) => {
  ...
  for (const file of files) {
    for (const link of stepLinks(file.text, file.own, names)) {
      if (!link.target) {
        homeless.push(`${file.name}:${link.startLine + 1} «${link.phrase}» :: ${link.around}`);
        continue;
      }
...
describe('ссылки по номерам шагов', () => {
  ...
  it('несуществующий номер назван вместе с файлом и адресатом', () => {
    ...
    expect(stepLinkFaults(files, steps).broken).toEqual([
      `вымысел.mjs:1 «${linkText('шаг', 40)}» → obrazec.md`,
    ]);
  });
  ...
  it('ссылка вне файла правил называет файл', () => {
    // Внутри файла правил безымянная ссылка законна: свой файл известен,
    // и требовать называть его в каждой такой ссылке значило бы утяжелить
    // сорок мест ради нуля пользы. Вне их своего файла нет вовсе, и адресата
    // не знает ни машина, ни читатель.
    //
    ...
    // Одно из четырёх живых расхождений нашлось именно среди них: пояснение
    // о выборке команд ревью называло номер, сдвинутый на единицу, тогда как
    // `gh pr checks` стоит шагом 3 `review.md`.
    //
    // Цена решения известна и заплачена: четырнадцать безадресных ссылок —
    // девять в `permissions.mjs` и пять здесь — выправлены тем же изменением.
    expect(stepLinkFaults(supervisorFiles(), skillSteps()).homeless).toEqual([]);
  });

  it('безадресная ссылка названа вне файла правил, а внутри — нет', () => {
    // Текст один и тот же, разница только в том, у кого есть свой файл.
    const sentence = `Первое разбирает ${linkText('шаг', 1)}.`;
    const outside = { name: 'вымысел.mjs', own: null, text: sentence };
    const inside = { name: 'obrazec.md', own: 'obrazec', text: `${OBRAZEC}\n\n${sentence}` };
    const steps = new Map([['obrazec', parseSteps(OBRAZEC)]]);

    expect(stepLinkFaults([outside], steps).homeless).toHaveLength(1);
    expect(stepLinkFaults([inside], steps).homeless).toEqual([]);
  });
...
describe('этапы и скиллы', () => {
  ...
  it('правило на одну приставку выкладки из двух этап не открывает', () => {
    ...
    // `deploy.md`, — а сверка имён переменных окружения (шаг 7 `deploy.md`)
    // зовёт `ssh` без него.
  ...
  it('этапы, освежающие базу, подтягивают свежую главную ветку', () => {
    ...
    // `merge` нельзя: оно уже стоит шагом 10 `design.md`, разбором голой
    // формы, и сторож по одному слову зеленел бы при полностью пропавшем
    // правиле.
  ...
  it('ревью главную ветку в дерево не подтягивает', () => {
    ...
    // знаком. Расчёт `merge-tree` шага 4 `review.md` под неё не попадает:
    // там другая подкоманда, и дерева она не трогает вовсе — ровно поэтому
    // ревью и получает свежесть главной ею, а не слиянием.
