...
describe('этапы и скиллы', () => {
  ...
  it('выкладка приводит дерево к выкладываемой ревизии', () => {
    // Шаг 6 `deploy.md` приводит дерево к тому, что поедет на сервер,
    // и держится он на одном лишь тексте скилла. Вычистив его правкой, никто
    // ничего не заметит, а выкладка начнёт врать двумя способами сразу:
    //
    // - замер меряет дерево, а не ревизию. Playwright поднимает клиент
    //   и сервер ИЗ ДЕРЕВА, тогда как на сервер уезжает то, что названо ключом
    //   `--ref`. Расходятся — и зелёный замер говорит о коде, который никуда
    //   не едет;
    // - файлы сборки всегда берутся из дерева. `Dockerfile`,
    //   `docker-compose.yml`, `.dockerignore`, `.env.example` и каталог
    //   `docker` дописываются в архив поверх выгрузки из git. 25.08.2026
    //   на этом сгорела выкладка: новые исходники собирались старым
    //   `Dockerfile`, а ошибку искали в коде.
    //
    // Проверка своя, а не дописывание `deploy` пятым в перечень соседнего
    // сторожа «этапы, освежающие базу, подтягивают свежую главную ветку».
    // Запасная форма записана в `deploy.md` ДОСЛОВНО той же строкой, какой
    // мерит тот сторож, поэтому дописывание зеленеет немедленно — без единой
    // правки скилла — и стережёт одну треть шага. Открытыми остаются основная
    // перемотка и проверка `diff --stat`, а правка «за компанию» скорее
    // уберёт именно их: запасное слияние она примет за рабочий путь, а
    // проверку — за лишний шаг.
    //
    // Мерка — строка целиком, вместе с ключом `-C`. Обрезанная до слова
    // `merge` зеленела бы на пересказе и на упоминании в разделе запретов —
    // ровно та ошибка, от которой предостерегает соседний сторож.
    //
    // Формы меряются порознь, чтобы падение называло пропавшую: одна проверка
    // на «есть все три» сказала бы только «в deploy.md что-то не так».
    //
    // Довод под командами меряется тоже и порознь от них. Он объясняет, ЧЕМ
    // платится расхождение, — а объяснение, оставшееся без охраны, уходит
    // первым: следующая правка уберёт и команды, не встретив причины, по
    // которой они стоят. Мерки — два жирных зачина шага 6 `deploy.md`:
    // «замер меряет дерево, а не ревизию» и «файлы сборки всегда берутся
    // из дерева».
    //
    // Обе ловушки выбора подстрок сняты с текста, а не предположены — проба
    // 04.09.2026 по действующему `deploy.md`:
    //
    // - голый `--ref` мерить нельзя: он стоит в правилах выкладки дважды —
    //   в самом доводе и в команде запуска `node scripts/deploy.mjs`, — и сторож
    //   по нему зеленел бы при начисто вычищенном доводе;
    // - мерка про файлы сборки не смягчается отбрасыванием слова «всегда»
    //   из середины: почти та же фраза с переставленными словами стоит
    //   в образце отчёта тех же правил, и смягчённая мерка находила бы её,
    //   а не довод.
    //
    // Обе мерки взяты без звёздочек разметки и без точки нарочно. Жирность
    // и точка суть разметка, а не существо: правка, снявшая выделение, роняла
    // бы сторожа, ничего не сломав, — а ложное падение учит читателя чинить
    // тест вместо правила.
    //
    // Цена короткой мерки названа честно: пояснение под зачином — про
    // Playwright и про `Dockerfile`, которым 25.08.2026 сгорела выкладка, —
    // перенесено на несколько строк и подстрокой не ищется. Правка, оставившая
    // зачин без пояснения, сторожем не ловится. Мерка через перевод строки
    // завела бы нормализацию, которую пришлось бы сторожить саму.
    //
    // Проверено порчей 04.09.2026, по одной форме за раз: у каждой из трёх
    // строк — `git -C <дерево> merge --ff-only origin/main`,
    // `git -C <дерево> merge origin/main` и
    // `git -C <дерево> diff --stat origin/main HEAD` — убирался ключ `-C`,
    // и всякий раз падала именно эта проверка, называя в сообщении
    // испорченную строку и только её. Разом три порчи не ставились нарочно:
    // они дали бы одно падение и не показали бы, что мерки различимы.
    // Зелень сторожа, заведённого на текст, где мерка уже стоит, работающую
    // проверку от ищущей не то не отличает — отсюда и проба.
    //
    // Мерки довода проверены порчей 04.09.2026 тем же порядком, по одной
    // за раз: убирался пункт «замер меряет дерево, а не ревизию», затем пункт
    // «файлы сборки всегда берутся из дерева», — и всякий раз падала именно
    // эта проверка, называя убранную строку и только её.
    //
    // Тогда же поставлена проба ловушки, и она показывает не работу сторожа,
    // а цену смягчения: мерка про файлы сборки временно урезалась до `сборки
    // берутся из дерева` при убранном пункте довода — и сторож ЗЕЛЕНЕЛ, найдя
    // фразу в образце отчёта тех же правил, где слова переставлены. Мерка
    // возвращена, и возврат доказан повторной порчей: с полной меркой и тем же
    // убранным пунктом проверка снова красная. Осмотром тут не обойтись —
    // этот файл правится законно, изменённостью он не улика, а прогон
    // с ослабленной меркой зелен, и забытое смягчение уехало бы в коммит,
    // оставив сторожа ослабленным навсегда.
    const text = skillText('deploy');
    // Перечень зовётся мерками, а не формами: три первые записи суть команды,
    // две последние — проза довода, и общее у них одно — предмет охраны.
    const marks = [
    ...
    expect(
      marks.filter((mark) => !text.includes(mark)).map((mark) => `deploy.md: ${mark}`),
    ).toEqual([]);
  });
