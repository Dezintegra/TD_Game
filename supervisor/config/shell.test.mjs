 ...
 *
 * Условие Bash проверяется вместе с советом повторить команду: раздельные
 * слова могли бы пропустить прежний безусловный совет. Независимый отказ
 * и неизвестная причина требуют разных действий, поэтому потеря каждой
 * оговорки должна диагностироваться отдельно с именем скилла.
 */
const SHELL_NOTE_MARKS = [
  '## Оболочка команд',
  'Команды этапа зовут из PowerShell, и другой оболочки у этапа нет.',
  'Если разрешённая для PowerShell команда вызвана через Bash и отказ вызван выбором Bash, это означает не запрет действия, а не ту оболочку: повтори команду в PowerShell.',
  'Независимый предохранитель из `$permissionIndependentGuardsNote` сменой оболочки не исправляется; следуй действию из перечня.',
  'Если причина отказа не установлена, сначала сверь сообщение, оболочку, разрешения и область применимости перечня.',
...
const missingNoteMarks = (text) => {
  ...
  const missing = SHELL_NOTE_MARKS.filter((mark) => !flat.includes(mark));
  if (
    /Отказ разрешений на предписанной скиллом команде означает не запрет действия, а не ту оболочку/.test(
      flat.replaceAll('**', ''),
    )
  ) {
    missing.push('безусловное объяснение отказа выбором оболочки');
  }
  return missing;
};

const noteProblems = (stage, text) => missingNoteMarks(text).map((mark) => `${stage}.md: ${mark}`);
...
describe('единственная оболочка: скиллы', () => {
  ...
  it('установка переменной средствами PowerShell сторожа не тревожит', () => {
    // Присваивание синтаксически допустимо в PowerShell. Эта проверка тела
    // не доказывает разрешённость действия в среде: независимый предохранитель
    // описан в $permissionIndependentGuardsNote и синтаксисом не отменяется.
  ...
  it('оговорка об оболочке стоит в каждом скилле', () => {
    ...
    const guilty = [];
    for (const stage of NEEDS_SESSION) {
      guilty.push(...noteProblems(stage, skillText(stage)));
    }
    expect(guilty).toEqual([]);
  });
  ...
  it('полная оговорка с переносами проходит', () => {
    expect(noteProblems('implement', completeNote.replaceAll(' ', '\n'))).toEqual([]);
  });

  it.each([
    [
      'условие Bash',
      'Если разрешённая для PowerShell команда вызвана через Bash и отказ вызван выбором Bash, это означает ',
      2,
    ],
    [
  ...
  ])('потеря условия «%s» называет скилл', (_name, fragment, markIndex) => {
    expect(noteProblems('deploy', completeNote.replace(fragment, ''))).toEqual([
      `deploy.md: ${SHELL_NOTE_MARKS[markIndex]}`,
    ]);
  });

  it('возврат безусловного совета ловится даже рядом с полной оговоркой', () => {
    const oldAdvice =
      '**Отказ разрешений на предписанной скиллом команде означает не запрет\nдействия, а не ту оболочку.** Повтори ту же команду из PowerShell';
    expect(noteProblems('benchmark', `${completeNote}\n${oldAdvice}`)).toEqual([
      'benchmark.md: безусловное объяснение отказа выбором оболочки',
    ]);
  });
});
