...
describe('баланс: соотношения из игрового замысла', () => {
  ...
  it('штурмовик стреляет на четыре клетки', () => {
    // Число заказано владельцем продукта прямо, и сверяется оно
    // с замыслом, а не с постоянной: сравнение с
    // `BASE_UNIT_RANGE_CELLS` было бы тавтологией — величина сама
    // от неё и выведена.
    expect(assault.range).toBe(cellsToUnits(4));
  });

  it('базовая башня открывает огонь раньше штурмовика', () => {
    // Проверяется СООТНОШЕНИЕМ, а не сравнением с записанной пятёркой.
    // На разнице «башня достаёт раньше пехоты» держится весь размен
    // обороны против наступления, а сравнение с числом разъехалось бы
    // с этим правилом молча при первой же правке башни.
    //
    // Запас после подъёма дальности штурмовика — ровно одна клетка
    // вместо трёх. Правило от этого не изменилось, а цена его
    // нарушения выросла: сузить зазор ещё раз значит отменить размен
    // вовсе.
    expect(STRUCTURE_STATS[StructureKind.TowerBasic].range).toBeGreaterThan(assault.range);
  });

  it('снайпер и Тесла перестреливают штурмовика', () => {
    // Соседняя проверка про «перестреливают ГЕНЕРАЛА» этого не
    // закрывает: там сравнение с другой величиной. Отрыв дальнобойных
    // от штурмовика ужался с четырёх клеток до двух, и до сих пор
    // его не стерёг никто.
    expect(sniper.range).toBeGreaterThan(assault.range);
    expect(tesla.range).toBeGreaterThan(assault.range);
  });
});
...
describe('баланс: ветки прокачки', () => {
  ...
  it('цена уровня растёт по трём ставкам: 10 обычные, 25 пороговые, 20 ядерные', () => {
    // Ускоренный рост цены — признак ветки, у которой уровень меняет
    // не количество, а качество: у дальности юнита это выход за круг
    // ответного огня.
    //
    // Экономика из этого списка ушла. Двадцать пять процентов стояли
    // у неё по доводу «усиливает сама себя, значит дорожать должна
    // быстрее всех»; замер показал, что при них цена обгоняла доход
    // и вложение переставало возвращаться задолго до конца матча.
    // Обоснование — при `INCOME_COST_PERCENT_BY_DESIGN`.
    //
