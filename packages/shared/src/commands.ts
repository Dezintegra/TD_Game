...
export const RejectReason = {
  ...
  /**
   * Ветка прокачки достигла своего потолка.
   *
   * Причина своя, а не «недопустимый довод». `InvalidArgument` означает
   * «такой ветки не существует» — это поломка клиента, и игроку
   * сообщать нечего. «Уровень предельный» означает «здесь всё,
   * вкладывай в другое»: команда была совершенно осмысленной,
   * и отказ приходит по игровому правилу.
   */
  UpgradeMaxed: 13,
  /**
   * Пусковая установка ядерного удара ещё не остыла.
   *
   * Причина своя, а не общая с нехваткой энергии, и разводит их то, что
   * игроку делать дальше. «Не хватает энергии» означает «копи».
   * «Не остыла» означает «подожди», и игроку с полным кошельком первый
   * ответ был бы прямой ложью.
   */
  NukeOnCooldown: 14,
} as const;
