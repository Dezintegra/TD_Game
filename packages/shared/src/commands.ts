...
export const RejectReason = {
  ...
  MatchOver: 10,
  /**
   * Клетка внутри защищённого кольца вокруг базы — чьей угодно.
   *
   * Причина своя, а не общая с «клетка занята», и это не мелочность.
   * «Занято» означает «целься в другое место»: уйдёт юнит — и всё
   * получится. «Слишком близко к базе» означает «здесь нельзя
   * в принципе, отойди». Действия у игрока разные, значит и причины
   * разные.
   */
  TooCloseToBase: 11,
  /**
