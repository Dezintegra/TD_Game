...
/**
 * Границы для величин, считаемых в штуках, а не в миллисекундах.
 *
 * Нужны там, где меряется «на сколько тактов сдвинули команду» или
 * «сколько команд было в пути»: миллисекундные границы там означали бы
 * бессмыслицу, а бюджет тика — тем более.
 *
 * Живут здесь, а не у каждого прибора своим списком, по одной
 * практической причине: показания клиента складываются с серверными
 * слиянием корзин, а слияние требует **совпадения границ**. Два списка
 * в двух файлах однажды разъедутся, и отчёт начнёт молча отвергаться.
 */
export const COUNT_BOUNDS: readonly number[] = [0, 1, 2, 4, 8, 16, 32];

/**
 * Бюджет для счётных величин.
 *
 * Ноль, и это не заглушка. Превышением здесь считается любое ненулевое
 * значение: один сдвинутый такт — уже показанное игроку «не то».
 */
export const COUNT_BUDGET = 0;

/**
 * Границы для скачков картинки, в клетках.
 *
 * Начинаются с четверти клетки: скачок мельче глазу не виден, а порог,
 * опущенный ниже, ловил бы округление внутренних единиц. Дальше
 * удвоения — половина клетки, клетка, две.
 *
 * Здесь же, а не у прибора, по той же причине, что и `COUNT_BOUNDS`:
 * корзины клиента складываются с серверными, а слияние требует
 * совпадения границ.
 */
export const JUMP_BOUNDS_CELLS: readonly number[] = [0.25, 0.5, 1, 2, 4, 8];
