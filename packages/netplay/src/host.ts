...
export const createMatchHost = (options: MatchHostOptions): MatchHost => {
  ...
  return {
    ...
    submit(player, command) {
      // Завершённый матч — единственная причина отказать: класть команду
      // некуда, сыгранного не переписать.
      //
      // Матч, ещё не начавшийся, командам не отказывает, и это не мелочь.
      // Приветствие уходит участнику сразу по подключению, то есть раньше
      // прихода соперника: всё это время участник видит мир, считает себя
      // в игре и вправе действовать. Съеденная здесь команда уносила бы
      // с собой уплаченную энергию — ровно то, против чего написано
      // правило про опоздавшую команду.
      //
      // Форы никому: пока соперник не пришёл, время матча не идёт,
      // и команда пришедшего вторым ложится на те же ранние тики.
      if (phase === 'finished') return;
