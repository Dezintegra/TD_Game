 ...
 * клетки, и юнит, упёршийся в основание, оказывался бы от центра
 * на 2,8 клетки. При тогдашней дальности штурмовика в две клетки это
 * означало, что до собственной цели он не достаёт вообще никогда.
 * Ровно так и выглядела ошибка: армия доходила до базы и вставала
 * вокруг, не нанося ей урона.
 *
 * Дальность штурмовика с тех пор поднята до четырёх клеток, и на ней
 * граница четырёх клеток до основания базы 3 × 3 при осевом подходе
 * находится в 5,5 клетки от центра. Фактическая остановка допускает
 * заход внутрь радиуса на один шаг; по диагонали пересчёт иной.
 * Мерка до основания при этом нужна ровно так же — она и делает
 * «четыре клетки до базы» осмысленной величиной.
 *
 ...
 *
 * Четыре этапа, и порядок их переставить нельзя:
 *   1. посадка скальных массивов с обязательным зазором между ними;
 *   2. раскрытие проходов до трёх клеток;
 *   3. расстановка баз с расчисткой площадок;
 *   4. проверка связности и аварийный коридор, если она нарушилась.
 *
 * Этап 2 — единственный, который превращает землю в скалу, поэтому все
 * этапы, добавляющие землю, идут после него. Обратный порядок заставил бы
 * проверять на узость площадку у базы и коридор, а так проверять нечего:
 * добавление земли узкого места не создаёт. Клетка, у которой квадрат
 * три на три был, его не теряет, когда рядом становится просторнее,
 * а клетки площадки девять на девять и коридора шириной три свои квадраты
 * имеют внутри себя.
 *
...
/**
 * Раскрытие проходов до трёх клеток — морфологическое открытие ядром
 * три на три.
 *
 * Два прохода по карте. Первый помечает клетки, вокруг которых весь
 * квадрат три на три проходим. Второй объявляет проходимыми клетки,
 * накрытые квадратом вокруг хоть одной помеченной; всё остальное
 * становится скалой.
 *
 * Зазор между массивами делает основную работу и без этого этапа, но
 * до двух случаев он не достаёт. Первый — щель между массивом и краем
 * карты: край не массив, зазор от него никто не держит. Второй — массив,
 * выросший подковой: горло между его же концами зазором не считается,
 * потому что массив там один.
 *
 * За краем карты — непроходимо, поэтому щель у края зарастает, а массив
 * прижимается к краю вплотную. Иначе вдоль периметра шла бы полоса,
 * по которой не пройти, но которую видно.
 *
 * Симметрия сохраняется сама: правило локально и одинаково во всех
 * направлениях, поэтому симметричная карта переходит в симметричную.
 *
 * Заращивание, а не расширение прохода. Можно было бы, наоборот, сносить
 * скалы там, где узко, но снос ломает форму массива, ради которой всё
 * и затевалось, и требует повторной проверки: не появилось ли узкое место
 * по соседству. Заращивание сходится за один проход и форму не портит.
 */
export const widenPassages = (cells: Uint8Array): void => {
  const spacious = new Uint8Array(MAP_CELL_COUNT);

  for (let y = 1; y < MAP_HEIGHT_CELLS - 1; y += 1) {
    for (let x = 1; x < MAP_WIDTH_CELLS - 1; x += 1) {
      if (isSquareClear(cells, x, y)) spacious[cellIndex(x, y)] = 1;
    }
  }

  const widened = new Uint8Array(MAP_CELL_COUNT).fill(Terrain.Rock);

  for (let y = 1; y < MAP_HEIGHT_CELLS - 1; y += 1) {
    for (let x = 1; x < MAP_WIDTH_CELLS - 1; x += 1) {
      if (spacious[cellIndex(x, y)] === 0) continue;

      for (let dy = -1; dy <= 1; dy += 1) {
        for (let dx = -1; dx <= 1; dx += 1) {
          widened[cellIndex(x + dx, y + dy)] = Terrain.Ground;
        }
      }
    }
  }
...
/** Весь ли квадрат три на три вокруг клетки проходим. */
const isSquareClear = (cells: Uint8Array, cx: number, cy: number): boolean => {
  for (let dy = -1; dy <= 1; dy += 1) {
    for (let dx = -1; dx <= 1; dx += 1) {
      if (cells[cellIndex(cx + dx, cy + dy)] !== Terrain.Ground) return false;
    }
  }
