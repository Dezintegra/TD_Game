...
describe('детерминизм симуляции', () => {
  ...
  it('совпадает с эталонной контрольной суммой', () => {
    ...
    //
    // Последним рост за убийства стал ветеранскими рангами: вместо
    // непрерывных пяти процентов сложным процентом — пять ступеней
    // с общими порогами 1/3/6/10/15 и двумя таблицами силы (башне
    // до удвоения, машине до полутора), и ранги теперь набирают ещё
    // и юниты. Правило боя изменилось прямо, но сумма сдвинулась бы
    // и без единого выстрела: поле `kills` входит в неё у каждой
    ...
    //
    // Предыдущие эталоны: 1152514732 — ядерные ветки прокачки;
    ...
    const GOLDEN_CHECKSUM = 2067497138;
