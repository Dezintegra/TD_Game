...
describe('детерминизм симуляции', () => {
  ...
  it('совпадает с эталонной контрольной суммой', () => {
    ...
    //
    // И последним появилось расталкивание: у машины завёлся личный радиус,
    // и после движения строй разводится, чтобы войско не слипалось
    // в точку. Правило меняет положения юнитов, а положения входят в сумму,
    // поэтому она обязана была сдвинуться. Полей в состоянии мира при этом
    // не прибавилось: смещение выводится из положений каждый тик.
    //
    // Предыдущие эталоны: 4262254141 — скорострельность генерала;
    ...
    const GOLDEN_CHECKSUM = 871233069;
