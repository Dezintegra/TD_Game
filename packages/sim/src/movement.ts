...
import { hasHostileInSight, seesStructure, unitElevation } from './combat.js';
 ...
 * ломает, либо — В РЕЖИМЕ «БОЙ» — в радиусе есть живой противник,
 * по которому он дострелит. Во всех прочих случаях он идёт, в том числе
 * ведя огонь на ходу.
 *
 * Вражеская постройка на пути юнита не останавливает, и это правило —
 * то, ради чего существует Тесла: её дальность на клетку больше
 * дальности базовой башни, поэтому, остановившись у назначенной цели,
 * она расстреливает эту цель, оставаясь вне досягаемости. Если бы юниты
 * останавливались у любой постройки, она залипала бы на первой встречной
 * стене и никогда не доходила до назначенной цели.
 *
 * Живой противник останавливает только в режиме «Бой». Правило верное —
 * две армии, прошедшие друг сквозь друга и разошедшиеся по своим делам,
 * это не тактика, а отсутствие события, — но как ЕДИНСТВЕННОЕ оно
 * означало, что осадная волна вязнет в любом заслоне, и решать это
 * за игрока не за что. Требование видимости здесь обязательно: без него
...
export const moveUnits = (
  ...
  for (const unit of working.units) {
    ...
    if (
      ...
      seesStructure(working, unitElevation(unit.unitType).structures, origin, target)
    ) {
    ...
    if (
      ...
      hasHostileInSight(working, hostiles, unit.owner, origin, baseline.range)
    ) {
      continue;
    }
