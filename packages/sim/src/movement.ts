 ...
 * его атаки, либо следующий шаг упирается в преграду, которую он ломает.
 ...
 * у назначенной цели, он расстреливает её, оставаясь вне досягаемости.
 * Если бы юниты останавливались у любой цели, он залипал бы на первой
 * встречной постройке и никогда не доходил до назначенной.
 */
export const moveUnits = (working: Working, statsTable: readonly PlayerStats[]): void => {
  ...
  for (const unit of working.units) {
    ...
    if (
      ...
      // Цель в радиусе — дальше идти незачем, работаем отсюда.
      continue;
    }
 ...
 */
export const findFreeCellNear = (working: Working, centre: number, maxRadius: number): number => {
  if (working.occupancy.blocked[centre] !== 1) return centre;
  ...
  for (let radius = 1; radius <= maxRadius; radius += 1) {
    for (let dy = -radius; dy <= radius; dy += 1) {
      for (let dx = -radius; dx <= radius; dx += 1) {
        ...
        if (working.occupancy.blocked[cell] !== 1) return cell;
      }
    }
  }
