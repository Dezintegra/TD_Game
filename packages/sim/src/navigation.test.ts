...
describe('поле расстояний', () => {
  ...
  it('обходит препятствие, а не проходит сквозь', () => {
    ...
    // приходится сделать крюк к единственному проходу и вернуться.
    //
    // Порог считается от прямого расстояния, а не вписан числом. Вписанное
    // «больше двадцати» было снято на карте 48 × 48, где проход лежал
    // на y = 24; на карте 38 × 38 он переехал на y = 19, крюк укоротился
    // ровно до двадцати, и проверка упала, ничего при этом не поймав.
    const direct = 10;
    const behind = distances[cellIndex(0, 10)] ?? UNREACHABLE;
    expect(behind).not.toBe(UNREACHABLE);
    expect(behind).toBeGreaterThanOrEqual(2 * direct);
  });
