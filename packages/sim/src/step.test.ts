...
describe('ядерный удар', () => {
  ...
  it('добычи за убитых взрывом не платится', () => {
    // У взрыва нет стрелка, и идёт он мимо `dealDamage` — через
    // `damageEntity`. Именно поэтому награда за убийство не достаётся
    // ему сама собой: подделать стрелка нечем, индекс пришлось бы
    // выдумать. Проверка стережёт разведённость двух этих путей.
    //
    // Прибавка обязана равняться одному лишь доходу тика — ни полной
    // добычи генерала, ни доли постройки. Удар накрывает многих сразу,
    // и заплати он хоть за одного, один залп окупал бы сам себя.
    const world = crowd(armed(richWorld(), CENTRE), CENTRE, UnitType.Assault, [1, 1]);
    const before = world.players[0]?.energy ?? 0;
