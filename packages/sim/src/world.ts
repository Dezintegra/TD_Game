...
export interface PlayerState {
  ...
  /**
   * Тик, раньше которого ядерный удар пустить нельзя.
   *
   * Ноль означает «установка готова», и матч начинается именно с него:
   * первый удар откатом не задержан.
   *
   * Тик готовности, а не счётчик оставшегося. Счётчик пришлось бы
   * уменьшать каждый тик у каждого игрока — то есть завести работу там,
   * где её нет; сравнение же не стоит ничего. Тем же способом устроены
   * перезарядка постройки и возрождение генерала, и третий способ
   * для той же величины заводить незачем.
   */
  readonly nukeReadyAtTick: TickNumber;
}
...
export interface NukeState {
  ...
  /**
   * Радиус поражения и мощность заряда ИМЕННО ЭТОЙ ракеты, посчитанные
   * в момент пуска из прокачки владельца.
   *
   * Хранятся здесь, а не выводятся при взрыве, по трём причинам, и первая
   * из них не игровая. Клиент переигрывает тики при откате предсказания;
   * если бы мощность читалась в момент взрыва, последовательность
   * «пуск, покупка уровня, взрыв», переигранная без дошедшей покупки,
   * дала бы другой взрыв при тех же командах пуска — расхождение
   * на ровном месте.
   *
   * Игровая причина: ракета уже сошла с направляющей, и прокачка,
   * купленная за три секунды её полёта, заряд не догоняет.
   *
   * Третья причина — отрисовка: кольцо поражения на земле рисуется
   * по этому радиусу, и лезть за ним в прокачку владельца из кода
   * отрисовки метки не приходится.
   */
...
export interface BlastState {
  ...
  readonly expiresAtTick: TickNumber;
  /**
   * Радиус поражения — только у ядерного взрыва, у прочих ноль.
   *
   * Появился здесь потому, что радиус перестал быть константой: вспышка,
   * ударная волна и гриб масштабируются от него, а у каждого удара
   * он теперь свой. Записи о взрывах в контрольную сумму не входят,
   * поэтому поле её не двигает.
   */
  readonly radius: number;
}
...
export const createWorld = (seed: number): WorldState => {
  ...
  const players: PlayerState[] = Array.from({ length: PLAYERS_PER_MATCH }, (_unused, index) => ({
    ...
    // Бой по умолчанию. Прежде здесь стоял Прорыв — от него войско
    // доходило до цели целиком, — но умолчание досталось не тому режиму:
    // «Бой» описывает то, чего игрок ждёт от войска по умолчанию (встречный
    // враг получает ответ), а «Прорыв» — намеренный приказ проигнорировать
    // заслон и дойти. Умолчанием обязано быть ожидаемое, а не выгодное
    // в осаде: не включив режим ни разу, игрок видел, как его войско
    // расстреливают на ходу, и объяснения этому в интерфейсе не было.
    stance: AttackStance.Engage,
    queue: [],
    // Установка готова с первого тика: откат разделяет пуски, а не
    // отделяет первый пуск от начала матча.
    nukeReadyAtTick: asTickNumber(0),
  }));
