...
/**
 * Порядок трат, в котором покупка с большей прибавкой обходит очередь.
 *
 * Переставляются только те пункты, у которых прибавка выражается уроном.
 * Прочие — прокачка в фазе, где интересна одна экономика, — остаются
 * на своих местах: сравнивать их не с чем, и двигать их значило бы
 * решать за профиль.
 */
export const orderBySpendGain = (
  ...
  const movable = order.filter((spending) => efficiency[spending] !== undefined);
  if (movable.length < 2) return order;

  const sorted = [...movable].sort(
    (left, right) => (efficiency[right] ?? 0) - (efficiency[left] ?? 0),
  );

  let next = 0;

  return order.map((spending) => {
    if (efficiency[spending] === undefined) return spending;

    const replacement = sorted[next] ?? spending;
    next += 1;
