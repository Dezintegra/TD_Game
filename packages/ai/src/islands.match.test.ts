...
describe('островной профиль', () => {
  ...
  it('башни ползут от своей базы к чужой, а не топчутся у горла', () => {
    ...
    //
    // РАЗЛИЧАЕТ ИХ ПРОДВИЖЕНИЕ, А НЕ РАЗБРОС, и это второе исправление
    // той же проверки. Прежде здесь сравнивалось среднее расстояние
    // между парами башен: 8,3 клетки у островного против 2,3 у базового,
    // с порогом «вдвое». Мерка развалилась при подъёме дальности
    // штурмовика до четырёх клеток — но развалилась НЕ оттого, что
    // сломалась доктрина.
    //
    // Сломался контроль. Радиус прикрытия генерала выведен из дальности
    // штурмовика и вырос с семи клеток до девяти; базовый профиль от
    // этого перестал двигать производство юнитов в начало порядка трат,
    // его генерал пошёл дальше, а башни он ставит там, где генерал
    // стои́т. Разброс базового вырос с 2,3 до 5,7 клетки, у островного
    // остался прежним (7,2), и порог «вдвое» стал недостижим при живой
    // и работающей доктрине. Понижать порог до 1,25 значило бы подогнать
    // число под результат и оставить проверку без запаса.
    //
    // Поэтому меряется то, о чём доктрина и говорит: скопление ПОЛЗЁТ
    // к чужой базе. Вторая половина поставленных башен обязана лежать
    // дальше от своей базы, чем первая, — и мерка эта к разбросу
    // равнодушна. Замер на этом зерне: +6,1 клетки у островного против
    // −1,2 у базового, то есть базовый не ползёт никуда вовсе.
    ...
    /**
     * Насколько дальше от своей базы вторая половина башен, чем первая.
     *
     * Расстояние берётся вдоль вероятного пути (`fromHome`), а не по
     * прямой: доктрина работает островами, расставленными по пути,
     * и мерить её надо той же линейкой.
     */
    const approach = approachOf(createWorld(SEED), ME);
    if (approach === undefined) throw new Error('нет вероятного пути');

    const creep = (list: readonly number[]): number => {
      const meanFromHome = (part: readonly number[]): number =>
        part.reduce((sum, cell) => sum + (approach.fromHome[cell] ?? 0), 0) /
        Math.max(part.length, 1);

      const half = Math.floor(list.length / 2);
      return meanFromHome(list.slice(list.length - half)) - meanFromHome(list.slice(0, half));
    };
    ...
    // Ползёт вперёд, а не назад и не на месте.
    expect(creep(mine)).toBeGreaterThan(0);

    // И контроль: базовый профиль так не делает. Запас в четыре клетки
    // взят с большим отступом от замеренных семи с лишним, но заведомо
    // больше того, что даст случай, — доктрина обязана быть видна,
    // а не едва угадываться.
    expect(creep(mine)).toBeGreaterThan(creep(theirs) + 4);
  });
