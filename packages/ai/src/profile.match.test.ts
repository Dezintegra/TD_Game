...
describe('прикрытие генерала считается рядом с ним', () => {
  it('радиус соседства равен сумме дальностей генерала и юнита', () => {
    // Пять клеток у генерала плюс четыре у штурмовика дают девять:
    // на таком расстоянии юнит и генерал достают до одного противника,
    // то есть действительно дерутся вместе. Величина выводится,
    // а не задаётся, и потому подросла сама — сперва вместе с дальностью
    // генерала, теперь вместе с дальностью штурмовика.
    //
    // Прежде здесь было записано «пять клеток у генерала плюс две
    // у штурмовика дают семь». Семёрка была верна, а разбор рядом
    // с ней — нет: он говорил про две клетки у ОБОИХ.
    const player = createWorld(SEED).players[AI];
    if (player === undefined) throw new Error('нет игрока');

    expect(escortRadius(playerStats(player))).toBe(cellsToUnits(9));
  });
...
describe('накопление уступает очередь тому, что выгоднее', () => {
  ...
  it('накопление на экономику не запирает прочие траты', () => {
    // Здесь стояла проверка обратного: «в ранней фазе интересна одна
    // экономика, её прибавку сравнивать не с чем, и такое накопление
    // обязано отложить всё, что стоит за ним».
    //
    // Посылка была верна, а следствие оказалось бедой. Ранняя фаза
    // базового профиля качает ОДНУ экономику; пока её прибавку нельзя
    // было оценить, порог накопления становился бесконечным, и до
    // постройки с машиной очередь не доходила ни разу за всю фазу.
    // Именно это владелец продукта видел как «компьютер стоит
    // и ничего не делает».
    //
    // Прибавка экономики теперь считается той же меркой, что и боевая,
    // поэтому порог конечен, и более выгодная покупка его обходит.
    // Само правило «несравнимое непревосходимо» осталось в силе — просто
    // ранняя фаза перестала быть его примером.
    ...
    // Копить в ранней фазе противник обязан по-прежнему: без накопления
    // проверка ниже была бы пустой.
    expect(waitedOnUpgrade.length).toBeGreaterThan(0);

    const overtook = waitedOnUpgrade.filter(({ record, at }) =>
      record.attempts.slice(at + 1).some((attempt) => attempt.result === 'bought'),
    );

    // «Хоть раз», а не «всегда»: обойти очередь может только то, что
    // и по карману, и выгоднее. Прежде таких решений не было НИ ОДНОГО.
    expect(overtook.length).toBeGreaterThan(0);
  });
});
