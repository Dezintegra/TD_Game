 ...
 * вместе. При нынешних пяти клетках у генерала и четырёх у штурмовика
 * получается девять.
 *
 * Разбор этот врал до сегодняшнего дня: здесь было записано «при
 * нынешних двух клетках у обоих получается четыре», тогда как генерала
 * перевели на пять ещё изменением `extend-general-reach`. Сама формула
 * при этом всё время оставалась верной — врало только объяснение,
 * и потому обе правки дальности прошли мимо него незамеченными.
 *
 * Величина растёт от прокачки дальности, и это правильно: дальнобойное
 * войско прикрывает с большего расстояния.
 *
 * Подъём дальности штурмовика с двух клеток до четырёх раздвигает этот
 * радиус с семи клеток до девяти, и последствие тут не косметическое:
 * признак «генерал прикрыт» выполняется чаще, а значит противник реже
 * двигает производство юнитов в начало порядка трат.
 *
 ...
 *
 * Заведён не как манера игры, а как **прибор**. Ветки дальности стрелка
 * появились в игре, и померить их влияние на арене нечем: внутри
 * выпавшей цели противник берёт самую дешёвую ветку, а дальность стоит
 * 240 и 480 при 60 и 120 у прочих веток тех же типов. Прочие догонят её
 * по цене примерно к пятнадцатому уровню каждая — почти тринадцать минут
 * дохода на один тип, — то есть в матче расчётной длины ни один
 * из существующих профилей не купит её ни разу. Замер на них показал бы
 * ровно ноль и был бы истолкован как «ветки ни на что не влияют».
 *
 * Чинить это здесь нельзя: оценкой выгоды покупок занимается изменение
 * `fix-ai-spending`, и там же записано требование про дорогую ветку.
 * До тех пор роль обходного пути играет `upgradeStats`.
 *
 * ## Из чего он собран
 *
 * **Поздняя фаза называет дальность.** Веса уходят к снайперу и Тесле,
 * а список характеристик — только дальность и добыча. Назови рядом
 * атаку, и дальность не купилась бы ни разу: список отсекает ветки
 * по характеристике, а не по цели, и внутри цели атака за 60 побеждает
 * дешевизной. Добыча названа отдельно, потому что у экономики
 * характеристика своя, и без неё выпавшая цель «база» не нашла бы
 * ни одной разрешённой ветки.
 *
 * Ядерного запаса поздняя фаза не держит — в отличие от базовой.
 * Иначе цена удара складывалась бы с ценой каждой покупки, и дальность
 * за 480 стала бы недостижимой ровно тем способом, ради проверки
 * которого прибор и заведён.
 *
 * **Терпения полторы минуты вместо сорока пяти секунд.** Дальность Теслы
 * стоит 480, то есть сорок восемь секунд базового дохода вместе с ценой
 * самой машины; при базовом терпении покупка объявлялась бы недостижимой
 * и отбрасывалась.
 *
 * ## Чего он НЕ умеет, и это надо знать до замера
 *
 * Прибор слаб в первые минуты: средняя фаза копит вместо того, чтобы
 * усиливать войско. Замер на пяти мирах против базового профиля
 * (20260820, 4242, 101, 7, 555):
 *
 * | мир      | матч      | дальность снайпера | дальность Теслы |
 * | -------- | --------- | ------------------ | --------------- |
 * | 20260820 | до конца  | 8                  | 5               |
 * | 555      | 18,9 мин  | 2                  | 0               |
 * | 101      | 16,9 мин  | 1                  | 0               |
 * | 4242     | 3,9 мин   | 0                  | 0               |
 * | 7        | 3,8 мин   | 0                  | 0               |
 *
 * То есть покупки идут там, где матч дожил до поздней фазы, и не идут
 * там, где прибор снесли раньше. Сводя цифры арены, помнить надо именно
 * это: доли побед у него меряют слабость средней фазы, а не силу веток.
 * Мерить по нему следует поведение — доходят ли Теслы до цели, сколько
 * их гибнет, сколько башен они снимают, — а не исход.
 */
export const LONG_RANGE_PROFILE: AiProfile = deepFreeze({
  ...BASELINE_PROFILE,
  id: 'long-range-2026-08',
  spending: { ...BASELINE_PROFILE.spending, savingHorizonSeconds: 150 },
  phases: [
    EARLY_ECONOMY,
    {
      // Вторая фаза копит доход и НЕ покупает боевых улучшений. Это
      // главное решение прибора, и оно измерено, а не предположено.
      //
      // Вариант, где средняя фаза взята у базового профиля целиком,
      // прогнан против него же на трёх мирах: за матч покупался ОДИН
      // уровень дальности. Казна держалась между двумя и шестьюдесятью
      // четырьмя единицами при цене дальности в 240 — башня за 60
      // и машина за 25 разбирали её раньше, чем она успевала дорасти.
      // Доход за тот матч поднялся всего на шесть уровней.
      //
      // Это та самая беда, которой занимается `fix-ai-spending`: дешёвое
      // выигрывает у выгодного просто тем, что по карману. Чинить её
      // здесь нельзя, а обойти в приборе — можно и нужно.
      untilSecond: 210,
      upgrades: { [UpgradeTarget.Base]: 1 },
      upgradeStats: [UpgradeStat.Income],
      mix: { [UnitType.Assault]: 3, [UnitType.Sniper]: 1, [UnitType.Tesla]: 0 },
      spend: ['upgrade', 'build', 'train'],
      reserve: 'none',
    },
    {
      untilSecond: Number.POSITIVE_INFINITY,
      upgrades: {
        [UpgradeTarget.UnitSniper]: 3,
        [UpgradeTarget.UnitTesla]: 3,
        // Экономика остаётся в весах, иначе доход встанет ровно тогда,
        // когда покупки станут дороже: у дальности рост цены двадцать
        // пять процентов за уровень, как у самой экономики.
        [UpgradeTarget.Base]: 2,
      },
      upgradeStats: [UpgradeStat.Range, UpgradeStat.Income],
      // Штурмовик в составе остаётся, и немного: ветки дальности у него
      // нет, и он здесь тем же, чем и в игре, — дешёвым прикрытием
      // для дорогих машин.
      mix: { [UnitType.Assault]: 2, [UnitType.Sniper]: 3, [UnitType.Tesla]: 3 },
      spend: ['upgrade', 'train', 'build'],
      reserve: 'none',
    },
  ],
