...
export const reserveOf = (
  ...
  //
  // И держит этот запас только манера, вкладывающаяся в ракету. Прежде
  // условие было не нужно: удар стоил полусотни стоимостей юнита
  // и при обычном доходе не набирался ни у кого, кроме терпеливых, —
  // запас сам собой оставался нулевым. Удар подешевел до шестнадцати,
  // и без условия его начали бы копить ВСЕ, то есть каждый профиль
  // стал бы держать в казне мёртвую сумму ради оружия, которое его
  // манера не применяет.
  //
  // Замер показал это прямо: базовый профиль, играющий соперником
  // в `siege.test.ts`, начал придерживать энергию, матч разошёлся,
  // и осадный перестал доходить до единственной за десять минут Теслы.
  const nukeReserve = profile.nuke.invest === true ? NUKE_COST : 0;
