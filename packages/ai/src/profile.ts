...
export interface AiProfile {
  ...
  readonly building: {
    ...
    /**
     * Во сколько ценится прикрытие своей башни стеной — выраженное
     * шириной горла, которое стоило бы столько же.
     *
     * Стена, поставленная в коридоре шириной в N клеток, перекрывает
     * 1/N подхода: одну клетку из трёх — треть, одну из тридцати —
     * ничего. Прикрытие башни в тех же долях и записывается: тройка
     * означает «прикрыть свою башню стоит ровно столько же, сколько
     * перекрыть горло в три клетки».
     *
     * Тройка не подобрана: уже трёх клеток проходов на карте не бывает
     * (§ 2.1 замысла — генерация заращивает такие места скалой). То есть
     * прикрытие башни назначено равным САМОМУ ценному перекрытию, какое
     * карта вообще допускает. Больше — и стены перестанут ходить в горло
     * вовсе; меньше — и перестанут прикрывать башни.
     */
    readonly shieldWorthCells: number;
    /**
     * Сколько мест перебрать, если выбранное заперло бы своим войскам
     * путь к цели.
     *
     * Ноль — прежнее поведение: отказ обрывает группу построек целиком.
     * Проверка достижимости стоит обхода карты, поэтому перебирать
     * помногу нельзя: каждая попытка это ещё один обход.
     */
...
export const BASELINE_PROFILE: AiProfile = deepFreeze({
  ...
  building: {
    ...
    shieldRadiusCells: 2,
    // Прикрытие башни стоит столько же, сколько перекрытие горла в три
    // клетки, — самого узкого, какое карта допускает.
    shieldWorthCells: 3,
