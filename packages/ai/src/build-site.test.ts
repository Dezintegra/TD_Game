...
describe('стена прикрывает свою башню', () => {
  it('стена встаёт со стороны подхода противника', () => {
    // Башня стреляет поверх стены, а пехота сквозь неё стрелять не может
    // (5.5), поэтому стена между башней и стороной подхода превращает
    // башню в укрепление.
    const bare = openWorld();
    const layout = approachOrThrow(bare);
    const stand = cellAtFraction(layout, 0.5);
  ...
  it('прикрывать нечего — места нет', () => {
    // Стена сама по себе лишь покупает противнику время на обход.
    // Ставить её без башни, которую она прикроет, незачем — и это
    // проверялось: правило «стена в узкое место коридора» было написано,
    // измерено и снято, см. комментарий к `shieldBuildCell`.
    const bare = openWorld();
    const layout = approachOrThrow(bare);
    const world = withGeneralAt(bare, cellAtFraction(layout, 0.5));

    expect(chooseWall(world, approachOrThrow(world), [])).toBe(-1);
  });
  ...
  it('отвергнутое место в перебор больше не попадает', () => {
    ...
    const first = chooseWall(world, approach, [mate]);
