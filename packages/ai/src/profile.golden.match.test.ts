 ...
 *
 * И наконец рост за убийства стал ветеранскими рангами (`earn-the-rank`).
 * Здесь сдвинулась сумма, а наблюдаемые величины не дрогнули ни на
 * единицу: построек по-прежнему шесть, войска двенадцать, прокачки
 * одиннадцать уровней, и генерал заходит на ту же долю пути.
 *
 * Так и должно быть. Ранги меняют ход БОЯ, а перечисленные величины
 * говорят о тратах и о маршруте генерала — то есть о решениях, которые
 * противник принимает до того, как выяснится, кто кого перестрелял.
 * Разойдись здесь ещё и они — это означало бы, что правка задела оценку
 * покупок, а не только бой, и разбираться стоило бы с этого.
 *
 * Предыдущие числа: 1337072980 — залп генерала;
 ...
 */
const GOLDEN_CHECKSUM = 1958727995;
