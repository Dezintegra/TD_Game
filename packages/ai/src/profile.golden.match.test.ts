 ...
 *
 * Последним пришло уменьшение карты (`compact-map-and-island-rocks`).
 * Здесь, в отличие от двух предыдущих правок, дрогнули и наблюдаемые
 * величины, и дрогнули заметно: построек четыре против шести, войска
 * десять против четырнадцати, прокачки одиннадцать уровней против
 * тринадцати, энергии на руках 1113 против 852, решений 115 против 94,
 * генерал доходит до 0,96 пути против 1,0222.
 *
 * Читается это так: на тесном поле противник копит, а не тратит.
 * Место под башню он оценивает по покрытию вероятного пути, а путь стал
 * короче — выгода каждой отдельной постройки упала, и очередь трат чаще
 * упирается в «коплю». Отсюда и лишние двести шестьдесят энергии
 * на руках, и лишние два десятка решений: решения принимаются, а тратами
 * не заканчиваются.
 *
 * Это не поломка профиля — правил его никто не трогал, — но это сдвиг
 * силы противника, и на глаз его засчитывать нельзя. Проверка ареной
 * заведена записью в `balance/queue.json`.
 *
 * Предыдущие числа: 2732107945 — карта 48 × 48;
 ...
 */
const GOLDEN_CHECKSUM = 1689291615;
