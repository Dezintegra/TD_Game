 ...
 *
 * Затем пришло расталкивание (`push-units-apart`). Манеру игры оно
 * не трогает вовсе: профиль не изменился ни одной величиной, а решения
 * принимаются по состоянию мира, в котором прибавилось только одно —
 * машины перестали стоять одна в другой. Но матч от этого пошёл иначе,
 * и наблюдаемые величины сдвинулись все сразу:
 *
 * - войско противника выросло с 12 машин до 14, а прокачка с 11 уровней
 *   до 13;
 * - команд стало заметно меньше — 94 против 154, — а энергии в конце
 *   вчетверо больше: 852 против 235.
 *
 * Все четыре числа говорят одно и то же с разных сторон: противник стал
 * меньше терять и потому меньше докупать взамен потерянного. Точную
 * причину этот тест не измеряет и утверждать её здесь нечестно; замер
 * изменения показал, что разведённое войско реже оказывается целиком
 * в радиусе одного стрелка. Генерал заходит чуть ближе — 1,0222 против
 * 1,0444, — но остаётся за прежним рубежом.
 *
 * Построек снова шесть: цены, сроки и оценка постройки не тронуты
 * и здесь.
 *
 ...
 */
const GOLDEN_CHECKSUM = 2287587834;
