...
/*
 * Девятьсот шестьдесят кончились в третий раз, и снова по той же причине:
 * награда башне за убийство (`pay-towers-for-kills`) удлинила матч.
 *
 * Замерено парой прогонов арены — ветка против главной, по шестьдесят
 * матчей на одном зерне, 33595620713 против 33595614304:
 *
 * | мерка                   | до    | после |
 * | ----------------------- | ----- | ----- |
 * | средняя длительность    | 12:49 | 13:32 |
 * | ничьих по времени       | 18,3% | 16,7% |
 * | башен потеряно за матч  | 83%   | 79%   |
 *
 * Читается это так. Опасались ОБРАТНОГО: что награда усилит того, кто и
 * так побеждает, и матчи станут короче. Тогда двигать окно было бы
 * бесполезно — осадный профиль просто не доживал бы до покупки, и лечить
 * пришлось бы правило, а не тест. Замер говорит противоположное: матч
 * стал длиннее на пять с половиной процентов, башни гибнут реже, а доля
 * упёршихся в предел времени не выросла. Значит случай прежний — окно
 * кончилось раньше события.
 *
 * Тысяча двести, а не тысяча с небольшим «по проценту темпа»: точный тик
 * первого заказа на этой ветке НЕ переснят. В рабочем дереве нет
 * зависимостей, а `pnpm install` отклоняется правилами разрешений, так что
 * зонда, каким мерили прошлые два раза, здесь не было. Запас поэтому взят
 * по привычке этого файла — измеренное плюс две минуты, — а не подогнан
 * впритык: темп замерен на зеркальной паре профилей, а стережёт число
 * поведение осадного против базового.
 *
 * Трём проверкам рывка ниже удлинение не вредит: у базового профиля
 * `baseShare: 1`, то есть рывка не случается ни на каком окне, а роевому
 * лишнее время только помогает.
 */
