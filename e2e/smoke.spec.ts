...
test('заказ пачки с зажатым модификатором даёт десять юнитов', async ({ page }) => {
  ...
  // Ждём именно счётчик штурмовиков, а не общее число войска.
  //
  // Раньше ждали общее, а состав по типам читали следом отдельным
  // действием — и между этими двумя чтениями войско успевало поредеть:
  // на поле 38 × 38 бой начинается раньше, чем на прежнем. Проверка
  // разваливалась не потому, что пачка не пришла, а потому, что она
  // уже начала гибнуть.
  await expect
    .poll(async () => number(page, 'own-unit-0'), { timeout: 8000 })
