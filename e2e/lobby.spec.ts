...
test('двое сходятся в комнате, и обоюдная готовность начинает матч', async ({ browser }) => {
   ...
   * Это потолок, а не стоимость.
   *
   * Цифры стоимости — с runner'а, а не со своей машины, и это важно:
   * прежние «20–34 с» были замером на рабочем ноутбуке и цену
   * занижали. На runner'е проверка занимает 30,1 с в спокойном прогоне,
   * 38,9 с в занятом и 51,7 с, когда идёт одновременно с тяжёлой
   * соседкой.
   *
   * Отсюда же ответ на вопрос, почему переопределение осталось после
   * того, как общий бюджет подняли до 90 с в `playwright.config.ts`.
   * Это не остаток, который забыли убрать. Проверка ведёт ДВА браузера
   * и потому дорожает от загруженности сильнее прочих: при её худших
   * 51,7 с девяноста хватило бы с запасом меньше двукратного, тогда
   * как у остальных проверок запас кратный. Убрать эту строку — значит
   * вернуть ровно ту тесноту, из-за которой всё и началось.
   */
  ...
  try {
    ...
    // от начала матча до первого подтверждённого тика проходят секунды,
    // и держит их НЕ сеть, а запекание рельефа на главном потоке клиента
    // (см. `Scene.bakeTerrain`). Сервер в это время шлёт кадры исправно —
    // они лежат в очереди браузера, пока поток занят.
    //
    // Прежде здесь стояло обратное: «цифра одинакова с видеокартой
    // и без неё, значит сетевая». Довод негодный: программный режим
    // замедляет растеризацию, а работа лежит в JavaScript, одинаковом
    // в обоих режимах, — сравнение было слепо к обвиняемому.
