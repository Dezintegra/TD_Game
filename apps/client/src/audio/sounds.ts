...
export const SOUND_PEAK: Readonly<Record<Sound, number>> = {
  ...
  [Sound.Arc]: 0.78,
  // Убавлен по решению владельца продукта на 35 % от прежних 0,72: выстрел
  // генерала звучал слишком громко. Доля приложена к пику, потому что пик —
  // единственная величина, которой громкость этого звука задаётся разом
  // и записи, и посчитанному запасному пути: загрузчик приводит к нему файл
  // (`engine.ts`), а `renderMissile` — посчитанный звук.
  // Ниже 0,42 уводить нельзя: там лежит самое тихое событие таблицы,
  // и провалившись под него выстрел генерала перестал бы читаться в бою,
  // а он событие рядовое — генерал стреляет весь матч.
  [Sound.Missile]: 0.468,
  ...
  // а исправление: ротор звучит НЕПРЕРЫВНО и висит над самим игроком
  // весь матч. При 0,34 он оказывался подложкой, поверх которой чужая
  // стрельба не читалась вовсе — то есть отнимал ровно то, ради чего
  // звук в игре и заведён.
  [Sound.Rotor]: 0.2,
};
 ...
 * Важность события.
 *
 * Нужна там, где звуков больше, чем мест: кого играть первым и кого
 * ради кого прерывать. Без неё случалось худшее, что может случиться
 * с игрой про ядерный удар, — сам удар не звучал: он гибнет
 * одновременно с полусотней юнитов, их хлопки занимали все места
 * раньше, и до него очередь не доходила.
 *
 * Порядок задан не громкостью, а тем, насколько событие меняет матч.
 * Гибель машины — самое частое и самое дешёвое, ядерный удар — самое
 * редкое и решающее.
 */
...
/**
 * С какой важности звук вправе прервать чужой, когда мест не осталось.
 *
 * Прерывать вообще — приём крайний: оборванный на полуслове взрыв
 * слышен обрывом. Но неслышный ядерный удар хуже любого обрыва.
 */
export const STEALS_FROM = 60;
