...
describe('отклик помещения', () => {
  ...
  it('хвост короток: поле, а не помещение', () => {
    // Порогом, а не равенством константе: проверять надо требование,
    // а не то, что число совпало само с собой. Полторы секунды хвоста
    // бывают только там, где есть стены, — и слышались они именно так.
    expect(REVERB_SECONDS).toBeLessThanOrEqual(0.5);
  });
