 ...
 * что занять его больше нечем: в его наборе только он, свист его же
 * ракеты и пуск, с которого всё началось.
 */
export const Pool = {
  Shots: 'shots',
  Blasts: 'blasts',
 ...
 * с лишним, и восьми одновременных хватает на любую свалку.
 *
 * Ядерных шесть, и число это выведено, а не выбрано: три вида звука
 * у ракеты (пуск, свист, удар) и две стороны, которые вправе пустить
 * ракету одновременно. Пока свист начинался с пуска, видов было два,
 * и двух мест хватало ровно на одну сторону; с тех пор у ракеты
 * появился пуск, а пуск и свист ПЕРЕКРЫВАЮТСЯ по времени — свист
 * приходит на последних двух третях подлёта, когда гул пуска ещё
 * не отзвучал. Оставь мы два места, второй игрок отнимал бы у первого
 * предупреждение о летящем в него ударе.
 */
