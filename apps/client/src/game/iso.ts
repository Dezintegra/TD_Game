...
/**
 * Во сколько раз круг на земле сплющен по экрану поперёк взгляда.
 *
 * Наклон проекции и есть это число: вдоль взгляда земля видна в полную
 * длину, поперёк — укороченной на синус наклона. Отсюда следствие,
 * о которое легко споткнуться: круг радиуса `r`, нарисованный НА ЗЕМЛЕ,
 * и круг того же радиуса, нарисованный НА ЭКРАНЕ, совпадают только
 * по ширине. По высоте экранный больше почти вдвое — и накрывает то,
 * что в мире осталось снаружи.
 */
export const GROUND_SQUASH = SIN_PITCH;
...
/**
 * То же направление, но с третьей осью.
 *
 * Плоского вектора хватает, пока все боковые грани отвесны: у такой грани
 * нормаль лежит в плоскости земли, и вертикальная составляющая взгляда
 * на знак произведения не влияет. Скошенной грани этого мало — она видна
 * и тогда, когда её горизонтальная нормаль смотрит от зрителя, лишь бы
 * сама грань была достаточно завалена кверху.
 *
 * Направление «к зрителю» — это то направление мира, которое проекция
 * переводит в точку: сдвиг вдоль него не меняет ни `x_s`, ни `y_s`.
 * Решая эту пару уравнений, получаем `(sin ψ, cos ψ, tg θ)`; после
 * приведения к единичной длине — вектор ниже.
 *
 * Горизонтальная часть сонаправлена `VIEW_DIRECTION`: тот пропорционален
 * `(sin ψ · sin θ, cos ψ · sin θ)`, этот — `(sin ψ · cos θ, cos ψ · cos θ)`.
 * Отличаются они только длиной, а для проверки знака длина безразлична,
 * поэтому на отвесных гранях оба теста дают одно и то же.
 */
export const VIEW_DIRECTION_3D = {
  x: SIN_YAW * COS_PITCH,
  y: COS_YAW * COS_PITCH,
  z: SIN_PITCH,
} as const;
