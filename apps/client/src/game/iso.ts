...
/**
 * Высота одной клетки на экране.
 *
 * Клетка проецируется в ромб, и его высота складывается из вертикальных
 * составляющих обеих мировых осей: `BX` даёт шаг по экрану при движении
 * на клетку по мировому `x`, `BY` — при движении по мировому `y`,
 * и обе положительны.
 *
 * Именно эту величину игрок видит как «одна клетка в высоту», и делением
 * на неё считается, сколько клеток помещается в поле по вертикали.
 * Число это не косметическое: при четырёх клетках стрелок и его мишень
 * на экране одновременно уже не помещаются ни для одного вида юнита.
 *
 * Считать по площади (`CELL_SCREEN_AREA_PX`) здесь нельзя: площадь
 * ничего не говорит о том, что творится вдоль вертикали, а узкое место
 * телефона — именно высота.
 */
export const CELL_SCREEN_HEIGHT_PX = BX + BY;
