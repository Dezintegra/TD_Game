 ...
 *
 * Факел ДЛИННЕЕ корпуса, и это не оговорка. Первая версия давала круглое
 * пятно вчетверо меньше — оно всё равно съедало ракету: свет, растущий
 * во все стороны, накрывает собой тело, и вместо ракеты остаётся летящая
 * искра. Направленный факел той же яркости тело не трогает вовсе,
 * потому что растёт назад, вдоль оси, — прочь от корпуса.
 *
 ...
 * по нему. Семнадцать оставляют факел длиннее корпуса, как и задумано,
 * но всю ракету укладывают примерно в длину танка.
 */
const PLUME_LENGTH_PX = 17;
const PLUME_PULSE_PX = 3.5;
...
const drawMissileBody = (
  ...
  // объектом на поле, а не выстрелом. Девять на 1,75 дают шестнадцать —
  // меньше половины машины, но вчетверо больше её ширины, поэтому ромб
  // остаётся ромбом и нос ещё виден.
  const body = 9;
  const half = 2.4;
  ...
  // Обводка тоньше прежней: на теле в пять пикселей поперёк линия в 1,2
  // съедала бы половину заливки, и ромб превратился бы в контур.
  layers.trails.stroke({ width: 1, color: accent, alpha: 0.95 });
  ...
  if (!lean) {
    glow.circle(nozzle.x, nozzle.y, 7.5);
  ...
  glow.circle(nozzle.x, nozzle.y, 2.2);
