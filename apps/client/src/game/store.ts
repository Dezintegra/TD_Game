...
export interface StatRow {
  ...
  /**
   * Уровень предельный: покупать больше нечего.
   *
   * Признак, а не цена-бесконечность. Цена у предельной ветки вообще
   * не имеет смысла — ядро отклонит покупку по любой, — и показывать
   * вместо неё число значило бы обещать то, чего интерфейс не сдержит.
   *
   * Ветка с потолком в игре одна, ядерный откат, но признак заведён
   * общим: следующая ветка с потолком иначе снова предложит кнопку,
   * которая гарантированно получит отказ.
   */
...
export interface MatchSnapshot {
  ...
  /**
   * Цена пуска и радиус поражения — с учётом прокачки.
   * Цена выводится из купленных уровней радиуса и мощности.
   */
  readonly nukeCost: number;
  readonly nukeRadiusCells: number;
  /**
   * Сколько секунд осталось ждать до следующего пуска. Ноль — установка
   * готова.
   *
   * Секунды, а не доля полосы: игрок решает, успеет ли накопить энергию
   * к следующему удару, и на этот вопрос отвечает число.
   */
  readonly nukeReadyInSeconds: number;
  /**
   * Строки характеристик по ГРУППАМ СТОЛБЦОВ, а не по целям прокачки.
   *
   * Для всех прежних плиток группа совпадает с целью, поэтому индекс
   * читается как `UpgradeTarget`. Исключение одно — ядерные ветки:
   * они принадлежат цели «база» (девятая цель сдвинула бы контрольную
   * сумму каждого мира), но показываются у плитки удара, где игрок
   * их и ищет. Под них заведена своя группа, см. `NUKE_STAT_GROUP`.
   *
   * Пустой список означает, что качать у этой плитки нечего.
   */
