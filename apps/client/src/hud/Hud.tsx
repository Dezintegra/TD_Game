...
const ConnectionLine = () => {
  ...
  return (
    ...
    >
      ...
      {/*
        Пометка стоит ради замера, а не ради теста разметки. Время оборота
        пакета, выросшее втрое при игре по localhost, означает не канал,
        а занятый главный поток: ответ на пинг лежит в очереди и
        приписывается связи. Рядом с частотой кадров это единственный
        дешёвый способ отличить «просела отрисовка» от «поток встал колом».

        В диагностические атрибуты величина не переезжает: там записано,
        что перечисленные там величины игроку текстом показываться
        MUST NOT, — а связь показывается и показываться обязана.
      */}
      <span data-testid="latency">связь {latency} мс</span>
      {inputTicks > 0 && (
...
const overlayStyle: CSSProperties = {
  ...
  // Выше панели прокачки, которая на маленьком экране лежит поверх поля.
  // Иначе меню — то самое, ради которого игрок и тянется к экрану, когда
  // всё пошло не так, — оказалось бы наполовину под панелью.
  zIndex: 4,
};
