...
арена — безголовые матчи и разбор поведения противника
  ...
  arena run [--matches N] [--seed N] [--profiles A,B] [--jobs N] [--seconds N]
            [--income K] [--speed K] [--tower-hp K] [--radius K] [--map K]
      Прогнать N матчей компьютер-против-компьютера. Матчи независимы
      и считаются параллельно по числу ядер.

      Пять последних ключей — множители правил на время прогона: базовый
      доход, скорость машин, прочность стреляющих построек, личный радиус
      машины и сторона карты. Единица означает «как задумано». Пересборки
      они не требуют, но и в игру не попадают: это инструмент замера.
