...
// Дежурных «в ожидании соперника» больше не бывает: они поднимаются
// по приглашению и уходят, отыграв. Показание осталось под прежним
// именем, но значит теперь другое — сколько дежурных поднято и ещё
// не начало матч, то есть идёт в комнату или сидит в ней до готовности.
// Постоянно ненулевым оно больше не бывает, и это норма, а не поломка.
metrics.gauge('td_computer_idle', 'Сколько дежурных поднято и ещё не начало матч', () =>
